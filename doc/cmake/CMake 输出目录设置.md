## 设置输出目录（旧方式）
```bash
set(EXECUTABLE_OUTPUT_PATH ${MY_BUILD_BINDIR})              # 设置所有可执行文件（add_executable）的输出目录（旧方式）
set(LIBRARY_OUTPUT_PATH ${MY_BUILD_BINDIR})                 # 设置所有库（共享/静态）的输出目录（旧方式）
set(CMAKE_LIBRARY_OUTPUT_DIRECTORY ${MY_BUILD_BINDIR})      # 设置共享库（SHARED）的输出路径（现代推荐）
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY ${MY_BUILD_LIBDIR})      # 设置静态库（STATIC）的输出路径（现代推荐）
```
...
```bash
set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG ${MY_BUILD_BINDIR})        # 构建的可执行文件输出路径
set(CMAKE_LIBRARY_OUTPUT_DIRECTORY_DEBUG ${MY_BUILD_BINDIR})        # 构建的共享库输出路径
set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_DEBUG ${MY_BUILD_LIBDIR})        # 构建的静态库输出路径，win下的导入.lib，导出.exp 文件也是这个路径
